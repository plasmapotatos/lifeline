...
export default function StatisticsPage() {
  const { data: statistics, isLoading, isError, error, refetch, isRefetching } = useStatistics();
    ...
    <div className="flex h-full flex-col gap-4 p-6 overflow-y-auto">
      ...
      {isError && (
        <div className="rounded-2xl border border-red-500/30 bg-red-500/10 p-6">
          <div className="text-lg font-semibold text-red-300 mb-2">Error Loading Statistics</div>
      ...
      {!isLoading && !isError && statistics ? (
        <>
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4">
            <div className="rounded-2xl border border-white/10 bg-gradient-to-br from-slate-900/80 to-slate-800/60 p-6 backdrop-blur-sm shadow-xl hover:shadow-2xl hover:shadow-slate-700/20 transition-all hover:border-slate-600/40 group">
              <div className="text-xs font-medium text-slate-500 uppercase tracking-wider mb-2">Total Events</div>
            ...
            <div className="rounded-2xl border border-white/10 bg-gradient-to-br from-slate-800/50 to-slate-700/30 p-6 backdrop-blur-sm shadow-xl hover:shadow-2xl hover:shadow-slate-700/20 transition-all hover:border-slate-600/40 group">
              <div className="text-xs font-medium text-slate-500 uppercase tracking-wider mb-2">Resolved Events</div>
            ...
            <div className="rounded-2xl border border-white/10 bg-gradient-to-br from-slate-800/50 to-slate-700/30 p-6 backdrop-blur-sm shadow-xl hover:shadow-2xl hover:shadow-slate-700/20 transition-all hover:border-slate-600/40 group">
              <div className="text-xs font-medium text-slate-500 uppercase tracking-wider mb-2">Active Emergencies</div>
            ...
            <div className="rounded-2xl border border-white/10 bg-gradient-to-br from-slate-800/50 to-slate-700/30 p-6 backdrop-blur-sm shadow-xl hover:shadow-2xl hover:shadow-slate-700/20 transition-all hover:border-slate-600/40 group">
              <div className="text-xs font-medium text-slate-500 uppercase tracking-wider mb-2">Fleet Size</div>
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            <div className="rounded-2xl border border-white/10 bg-gradient-to-br from-slate-800/50 to-slate-700/30 p-6 backdrop-blur-sm shadow-xl hover:shadow-slate-700/20 transition-all">
              <div className="text-sm font-medium text-slate-400 mb-1">Average Dispatch Time</div>
              <div className="text-3xl font-bold text-cyan-500/60 mt-2">{formatSeconds(statistics.avg_dispatch_time_seconds)}</div>
              <div className="text-xs text-slate-500 mt-1">Time from detected → dispatched</div>
            </div>

            <div className="rounded-2xl border border-white/10 bg-gradient-to-br from-slate-800/50 to-slate-700/30 p-6 backdrop-blur-sm shadow-xl hover:shadow-slate-700/20 transition-all">
              <div className="text-sm font-medium text-slate-400 mb-1">Average Response Time</div>
              <div className="text-3xl font-bold text-emerald-500/60 mt-2">{formatSeconds(statistics.avg_response_time_seconds)}</div>
              <div className="text-xs text-slate-500 mt-1">Time from dispatch → resolved</div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
            ...
            <div className="rounded-2xl border border-white/10 bg-gradient-to-br from-slate-900/80 to-slate-800/60 p-6 backdrop-blur-sm shadow-xl">
              <div className="mb-4 text-sm font-semibold text-slate-300 uppercase tracking-wide">Severity Breakdown</div>
              <ResponsiveContainer width="100%" height={250}>
                <PieChart>
                  <Pie
                    data={[
                      { name: "Emergency", value: statistics.severity_breakdown.emergency },
                      { name: "Informational", value: statistics.severity_breakdown.informational },
                    ]}
                    cx="50%"
                    cy="50%"
                    labelLine={false}
              ...
              <div className="mt-4 grid grid-cols-2 gap-4 text-center">
                <div className="p-3 rounded-xl bg-red-500/10 border border-red-500/20">
                  ...
                  <div className="mt-1 text-lg font-bold text-red-500/60">{statistics.severity_breakdown.emergency}</div>
                </div>
                <div className="p-3 rounded-xl bg-blue-500/10 border border-blue-500/20">
                  <div className="text-xs text-slate-500">Informational</div>
                  <div className="mt-1 text-lg font-bold text-blue-500/60">{statistics.severity_breakdown.informational}</div>
                </div>
              </div>
            </div>
            ...
            <div className="rounded-2xl border border-white/10 bg-gradient-to-br from-slate-900/80 to-slate-800/60 p-6 backdrop-blur-sm shadow-xl">
              <div className="mb-4 text-sm font-semibold text-slate-300 uppercase tracking-wide">Fleet Status Distribution</div>
              <ResponsiveContainer width="100%" height={250}>
                <BarChart
                  data={[
                    { 
                      status: "Free", 
                      count: statistics.fleet_overview.filter(a => a.status === "free").length
                    },
                    { 
                      status: "In Transit", 
                      count: statistics.fleet_overview.filter(a => a.status === "in_transit").length
                    },
                    { 
                      status: "Unavailable", 
                      count: statistics.fleet_overview.filter(a => a.status === "unavailable").length
                    },
                  ]}
                >
                  ...
                  <Bar dataKey="count" radius={[8, 8, 0, 0]}>
                    {[
                      statistics.fleet_overview.filter(a => a.status === "free").length,
                      statistics.fleet_overview.filter(a => a.status === "in_transit").length,
                      statistics.fleet_overview.filter(a => a.status === "unavailable").length,
                    ].map((_, index) => (
                      <Cell 
                        key={`cell-${index}`} 
                        fill={index === 0 ? "rgba(52, 211, 153, 0.7)" : index === 1 ? "rgba(251, 191, 36, 0.7)" : "rgba(248, 113, 113, 0.7)"} 
                      />
                    ))}
                  </Bar>
                </BarChart>
          ...
          <div className="rounded-2xl border border-white/10 bg-gradient-to-br from-slate-900/80 to-slate-800/60 backdrop-blur-sm shadow-xl overflow-hidden">
            <div className="p-6 border-b border-white/10">
              <div className="text-sm font-semibold text-slate-300 uppercase tracking-wide">Fleet Overview</div>
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-white/5">
                <thead className="bg-slate-950/60">
                  <tr>
                    <th className="px-6 py-4 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">Ambulance ID</th>
                    <th className="px-6 py-4 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">Status</th>
                    <th className="px-6 py-4 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">Current Event</th>
                    <th className="px-6 py-4 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">ETA</th>
                    <th className="px-6 py-4 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">Events Handled</th>
                  </tr>
                </thead>
                      ...
                      >
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <span className="text-sm text-slate-500 font-mono">
                            {ambulance.current_event_id ? ambulance.current_event_id.slice(-8) : "—"}
                          </span>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
                          <span className="text-sm text-slate-400">
                            {ambulance.eta_seconds ? formatSeconds(ambulance.eta_seconds) : "—"}
                          </span>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
                          <span className="text-sm font-semibold text-blue-500/60">{ambulance.events_handled}</span>
                        </td>
                      </tr>
                    ))
                  )}
        ...
        <div className="rounded-2xl border border-yellow-500/30 bg-yellow-500/10 p-6">
          <div className="text-lg font-semibold text-yellow-300 mb-2">No Data Available</div>
